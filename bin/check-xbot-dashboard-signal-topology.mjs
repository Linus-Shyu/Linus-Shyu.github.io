...
for (const dir of dashboards) {
  ...
  for (const section of ["overview", "decision", "language", "tasks", "funnel", "trend", "posts", "advice", "drafts", "cost", "evidence"]) {
    if (!html.includes(`id="${section}"`)) fail(`${dir}/index.html is missing growth topology section.`, section);
  }
  for (const token of ["renderHero", "renderGrowthDecision", "renderLanguageTracks", "renderFreshness", "renderTasks", "renderFunnel", "renderCharts", "renderPosts", "renderDrafts", "renderAdvice", "renderCost", "renderEvidence"]) {
